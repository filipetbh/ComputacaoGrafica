...
def convertToMedia(path, medianafilename):
    try:          
        img = cv2.imread(path)
        img_filtro_mediana = cv2.medianBlur(img, 11)
        cv2.imwrite(medianafilename, img_filtro_mediana)
        print ('Salvando a imagem:', medianafilename)
...
if __name__ == "__main__":
    ...
    for fn in glob.glob('*.jpg'):
            ...
            convertToMedia(fn, 'MedianaImages/' + f + '.jpg')
